...
if (!empty($_GET['estado']) || !empty($_GET['ciudad']) || !empty($_GET['optica'])){
    ...
    $resultOptica = $my_sql_conn->query("SELECT nombre from optica where id='$optica'");
    while ( $rs = $resultOptica->fetch_array(MYSQLI_ASSOC)) {
      $nombre = $rs['nombre'];
    }
  ...
  <div class="container">
    <div class="jumbotron">
        <div class="container-fluid">
            <div class="row">
              <h3>Reporte tus pedidos de <?php echo $nombre; ?></h3>
              <h5>Ubicacion: <?php echo $city; ?>, <?php echo $edo; ?></h5>
            </div>
            <div class="row">
              <table class="table">
                <thead>
                ...
                <tbody>
                  <?php
                    $resultBisel = $my_sql_conn->query("SELECT * from trabajos where OpticID='$optica'");
